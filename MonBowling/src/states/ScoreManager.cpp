...
void ScoreManager::initialize() {
    ...
    mScoreText->setPosition(20, 20);  // En haut à droite
    ...
    Ogre::OverlayContainer* textContainer = static_cast<Ogre::OverlayContainer*>(
        Ogre::OverlayManager::getSingleton().createOverlayElement("Panel", "ScoreTextContainer"));

    // Ajouter l'élément texte au conteneur
    textContainer->addChild(mScoreText);

    // Ajouter le conteneur à l'overlay
    mScoreOverlay->add2D(textContainer);

    // Afficher l'overlay
    mScoreOverlay->show();
/*    
    // Ajout à l'overlay
    mScoreOverlay->add2D(mScoreText);
